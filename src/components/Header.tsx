...
function Header() {
  ...
  return (
    <header className="[grid-area:header] border-b-2 border-primary-800 flex items-center justify-end">
      {session && <p className="text-xl mr-3">{session.user.name}</p>}
      {session && (
        <button
          onClick={odhlasit}
          type="button"
          className="mr-5 bg-primary-800 flex items-center p-2 rounded-2xl cursor-pointer hover:bg-primary-700 transition-all duration-200"
        >
          <FiLogOut size={30} />
        </button>
      )}
