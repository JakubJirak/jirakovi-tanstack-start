...
const TodayForecastHour = ({ time, icon, temp, rain }: WeatherData) => {
  return (
    <div className="border border-border bg-secondary rounded-xl flex flex-col items-center justify-between py-3 min-w-[150px] flex-shrink-0 overflow-hidden">
      <p className="text-lg text-gray-300">{time}</p>
      <div className="w-25 lg:w-[9vw] xl:w-30 m-[-24px]">
        <img
          width="100%"
          src={`/weather-icons-static/${icon}.svg`}
          alt={icon}
        />
      </div>
