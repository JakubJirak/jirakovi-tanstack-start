...
function RouteComponent() {
  const [brambory, setBrambory] = useState<number>(100);
  ...
  function vypocet(e: React.FormEvent<HTMLFormElement>) {
    ...
    setVysledek({
      mouka: brambory / 3,
      krupicka: (brambory * 2) / 15,
      vejce: brambory / 750,
    });
  }

  return (
    <div className="w-[95%] max-w-[500px] mx-auto mt-3 flex flex-col items-center justify-center">
      <form
        onSubmit={vypocet}
      ...
      >
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <input
            ...
            max="50000"
            value={brambory}
            onChange={(e) => setBrambory(Number(e.target.value))}
      ...
      {vysledek && (
        <div className="w-full mt-5 text-2xl flex flex-col gap-5 bg-primary-900 rounded-xl p-4">
          <div className="grid grid-cols-[50px_90px_1fr] items-center gap-5 w-65 mx-auto">
            <PiBowlFood size={48} />
            <p>Počet:</p>
            <p>{(brambory * pocetNasob).toFixed(1)} ks</p>
          </div>
          <div className="grid grid-cols-[50px_90px_1fr] items-center gap-5 w-65 mx-auto">
            <img src={flour} alt="" />
            <p>Mouka:</p>
